...
static void fg_cycle_counter_update(struct fg_dev *fg)
{
	...
	if (fg->batt_psy)
	{
		rc = power_supply_get_property(fg->batt_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
				&prop);
		if (rc < 0) {
