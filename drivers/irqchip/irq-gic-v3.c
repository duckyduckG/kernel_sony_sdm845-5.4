...
#define for_each_spi_irq_word(i, reg) \
	for (i = 0; \
	    i < DIV_ROUND_UP(IRQ_NR_BOUND(GIC_LINE_NR) - SPI_START_IRQ, \
...
void gic_v3_dist_save(void)
{
	...
	for (i = 32; i < IRQ_NR_BOUND(GIC_LINE_NR); i++) {
...
static void _gic_v3_dist_restore_set_reg(u32 offset)
{
	void __iomem *base = gic_data.dist_base;
	int i, j = SPI_START_IRQ, l;
	int irq_nr = IRQ_NR_BOUND(GIC_LINE_NR) - SPI_START_IRQ;
...
void gic_v3_dist_restore(void)
{
	...
	if (bitmap_empty(irqs_restore, IRQ_NR_BOUND(GIC_LINE_NR)))
		return;
