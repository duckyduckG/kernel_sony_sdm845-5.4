...
static int msm_vidc_load_bus_table(struct msm_vidc_platform_resources *res)
{
	...
	num_buses = of_property_count_strings(pdev->dev.of_node,
				"interconnect-names");
	if (num_buses <= 0) {
		dprintk(VIDC_ERR, "%s: No buses found", __func__);
		return -EINVAL;
	}

	buses->count = num_buses;
	dprintk(VIDC_INFO, "Found %d bus interconnects\n", num_buses);

	bus_ranges = kzalloc(2 * num_buses * sizeof(*bus_ranges), GFP_KERNEL);
	if (!bus_ranges) {
		dprintk(VIDC_ERR, "No memory to read bus ranges\n");
		return -ENOMEM;
	}
	...
	rc = of_property_read_u32_array(pdev->dev.of_node,
				...
				num_buses);
	if (rc) {
		dprintk(VIDC_ERR, "Failed to read bus ranges: defaulting to <0 INT_MAX>\n");
	...
	if (!buses->bus_tbl) {
		dprintk(VIDC_ERR, "No memory for bus table\n");
		rc = -ENOMEM;
		goto exit;
	}
