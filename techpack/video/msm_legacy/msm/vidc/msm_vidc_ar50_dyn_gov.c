...
static unsigned long __calculate_vpe_ar50(struct vidc_bus_vote_data *d,
		enum vidc_bus_type type)
{
	return 0;
}

static unsigned long __calculate_decoder_ar50(struct vidc_bus_vote_data *d,
		enum vidc_bus_type type)
{
	/*
...
static unsigned long __calculate_encoder_ar50(struct vidc_bus_vote_data *d,
		enum vidc_bus_type type)
{
	/*
	 * XXX: Don't fool around with any of the hardcoded numbers unless you
...
static unsigned long __calculate_ar50(struct vidc_bus_vote_data *d,
		enum vidc_bus_type type)
{
	...
	case HAL_VIDEO_DOMAIN_VPE:
		value = __calculate_vpe_ar50(d, type);
		break;
	case HAL_VIDEO_DOMAIN_ENCODER:
		value = __calculate_encoder_ar50(d, type);
		break;
	case HAL_VIDEO_DOMAIN_DECODER:
		value = __calculate_decoder_ar50(d, type);
		break;
	default:
				...
				struct msm_vidc_gov_data *vidc_data)
{
	...
	for (c = 0; c < vidc_data->data_count; ++c)
		ab_kbps += __calculate_ar50(&vidc_data->data[c], type);
